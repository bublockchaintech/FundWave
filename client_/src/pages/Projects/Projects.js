...
const Projects = ({ getProviderOrSigner }) => {
  ...
  return (
    <>
      ...
      {stageState === 2 && (
        <FundProject projects={projects} lastUpdate={lastUpdate} getProviderOrSigner={getProviderOrSigner} />
      )}
      {(stageState === 3 || stageState === 4) && (
        <ExecuteProject projects={projects} lastUpdate={lastUpdate} getProviderOrSigner={getProviderOrSigner} />
      )}
    </>
  );
};
